...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, images_list):
        ...
        xz_norm = []
        num = len(self.blocks)
        for i in range(num):  # batch is the first dimension.
            if self.use_checkpoint:
                xz = checkpoint.checkpoint(self.blocks[i], xz)
            else:
                xz = self.blocks[i](xz)
                xz_norm.append(self.norm(xz))

        xz = self.norm(xz)  # B,N,C
        out = {'xz': xz, 'all': xz_norm}
        return out
...
@register_model
def vit_light(pretrained=False, pretrain_type='default',
    ...
    if pretrained:
        load_pretrained(model, pretrain_type,
                        num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3), strict=False)
    return model
