...
from lib.utils import box_ops1
from lib.utils.misc import get_world_size, is_dist_avail_and_initialized
from lib.utils.box_ops1 import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, generalized_box_iou
from lib.utils import box_ops

class SetCriterionAllProposal(nn.Module):
    ...
    def loss_boxes(self, outputs, targets):
        ...
        query_num = outputs['pred_boxes'].size(1)
        ...
        for batch_idx in range(batch_size):
            ...
            pred_norm_box_list.append(bz_src_boxes / bz_image_whwh)  # normalize (x1, y1, x2, y2)
            tgt_box_list.append(bz_target_boxes.repeat(query_num, 1))
            tgt_box_xyxy_list.append(bz_target_boxes_xyxy.repeat(query_num, 1))
        ...
        if len(pred_box_list) != 0:
            ...
            loss_bbox = F.l1_loss(src_boxes_norm, box_cxcywh_to_xyxy(target_boxes))
            losses['loss_bbox'] = loss_bbox #.sum() #/ num_boxes

            loss_giou, iou = box_ops.giou_loss(src_boxes_norm, box_ops.box_cxcywh_to_xyxy(target_boxes))
            # loss_giou = 1 - torch.diag(box_ops1.generalized_box_iou(src_boxes, target_boxes_abs_xyxy))
            mean_iou = iou.detach().mean()
            losses['loss_giou'] = loss_giou.sum()
            losses['giou'] = mean_iou


            #with torch.no_grad():
                # a = torch.diag(box_ops1.generalized_box_iou(src_boxes, target_boxes_abs_xyxy))
                 #losses['giou'] = a.sum() / num_boxes
        else:
...
class SetCriterionDynamicK(nn.Module):
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        self.use_fed_loss = cfg.MODEL.LOSS.USE_FED_LOSS
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.LOSS.ALPHA
            self.focal_loss_gamma = cfg.MODEL.LOSS.GAMMA
        else:
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(pred_box_list) != 0:
            ...
            with torch.no_grad():
                 a = torch.diag(box_ops1.generalized_box_iou(src_boxes, target_boxes_abs_xyxy))
                 losses['giou'] = a.sum() / num_boxes
           # print(str(1- loss_giou.sum() / num_boxes))
...
class HungarianMatcherDynamicK(nn.Module):
    ...
    def __init__(self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, cost_mask: float = 1, use_focal: bool = False):
        ...
        self.use_focal = use_focal
        self.use_fed_loss = cfg.MODEL.LOSS.USE_FED_LOSS
        self.ota_k = cfg.MODEL.LOSS.OTA_K
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.LOSS.ALPHA
            self.focal_loss_gamma = cfg.MODEL.LOSS.GAMMA
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(bs):
                ...
                pair_wise_ious = ops.box_iou(bz_boxes, bz_gtboxs_abs_xyxy)  # [500, 4] [n, 4] -> [500, n]
                ...
                bz_out_bbox_ = bz_boxes / bz_image_size_out  # normalize (x1, y1, x2, y2) [500, 4]
                bz_tgt_bbox_ = bz_gtboxs_abs_xyxy / bz_image_size_tgt
                bz_out_bbox_ = bz_out_bbox_.to(torch.float32)# normalize (x1, y1, x2, y2)  [n, 4]
                cost_bbox = torch.cdist(bz_out_bbox_, bz_tgt_bbox_, p=1)  # [500, n]

                cost_giou = -generalized_box_iou(bz_boxes, bz_gtboxs_abs_xyxy)  # [500, n]
    ...
    def get_in_boxes_info(self, boxes, target_gts, expanded_strides):
        xy_target_gts = box_cxcywh_to_xyxy(target_gts)  # (x1, y1, x2, y2)  (5, 4)

        anchor_center_x = boxes[:, 0].unsqueeze(1)  # [500, 1]
        anchor_center_y = boxes[:, 1].unsqueeze(1)  # [500, 1]
