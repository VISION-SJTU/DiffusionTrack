...
from lib.models.diffusiontrack import vit as vit_module
...
class Encoder(EncoderBase):
    ...
    def __init__(self, name: str,
                 ...
                 open_layers: list,
                 cfg=None):
        if "vit" in name.lower() and 'hivit' not in name.lower():
            ...
            if "_base_" in name:
                ...
                num_channels = 768
        elif 'hivit' in name.lower():
            encoder = getattr(hvit_module, name)(pth=cfg.MODEL.ENCODER.PRETRAIN_TYPE,
                                                search_size=search_size, template_size=template_size,
                                                search_number=search_number, template_number=template_number,
                                                )
            if "base" in name:
