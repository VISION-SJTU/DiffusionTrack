...
def evaluate(model, images_list, xz, input_start, hanning, bs):
  ...
  #  custom_ops = {nn.MultiheadAttention: get_complexity_MHA}
    # encoder
    macs1, params1 = profile(model, inputs=(images_list, None, input_start, "encoder"), custom_ops=None ,verbose=True)
    macs, params = clever_format([macs1, params1], "%.3f")
    print('encoder macs is ', macs)
    print('encoder params is ', params)
    # decoder
    # xz, boxes, t\
    boxes = torch.randn([1, 50, 4], device=xz[0].device)
    t = torch.Tensor([999])
    t = t.to(device=xz[0].device)
    macs2, params2 = profile(model, inputs=(None, xz, boxes,  "decoder", t, ), custom_ops=None, verbose=True)
...
if __name__ == "__main__":
    ...
    if cfg.TEST.WINDOW == True:
        ...
        hanning = None
    '''import diffusiontrack network module'''
    model_module = importlib.import_module('lib.models.diffusiontrack')
    model_constructor = model_module.build_diffusiontrack
